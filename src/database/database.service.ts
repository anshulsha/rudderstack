...
export class DatabaseService {
  ...
  async updateOne(collection: string, whereClause: any, data: any) {
    ...
    const options = { new: true, versionKey: true }; // Set versionKey to true

    await this.models[collection].findByIdAndUpdate(whereClause, data, options);

    const fetch_document = await this.findOne(collection, whereClause);
